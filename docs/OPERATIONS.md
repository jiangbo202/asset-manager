...
- 只有 1 个触发器：**每小时**跑一次（`0 * * * *`）
- 实际干活的条件：当前时间**已到或已过**设置里配置的小时（默认 22 点，按设置里的时区算）
  1. 刷新行情（批量接口优先）
  2. 打当日快照
- 同一天不会重复打快照（拍完后续小时直接返回）
- **补拍**：过了配置小时而当天还没有快照，后续每个小时都会重试一次。
  所以到点那次就算被中断（例如超出免费版 CPU 限额），当天也能自动补上，不用等到第二天
- **持仓分批**：单次运行最多刷新 12 个持仓（`CRON_MAX_HOLDINGS`），按「最久没更新」排序，
  超出的留到下一次运行 —— 因为重活（行情 + 快照）的 CPU 开销会随持仓数量增长，而免费版每次调用只有 10ms。
  汇率不参与分批（折算总额依赖它）。快照照常拍摄，用这些标的当前已知的价格。
  设置页「最近运行」下方会提示上次留了多少个；点「立即刷新行情」不受上限约束
