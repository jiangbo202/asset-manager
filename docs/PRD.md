...
- **v1.5（实现中）**：砍掉 ECharts，改为**手写 SVG/CSS 图表**（环形图 + squarified treemap），前端产物 **81.5KB gzip**（原 244.6KB）；新增本地开发方案（`docs/LOCAL_DEV.md`）；M0 与 Spike 已跑通（详见 README 当前状态）。
...
| D2 | 外部行情**默认关闭**，开关点击后提示"下个版本支持"（纯占位） | 用户确认 |
...
| FR-6.3 | 市场筛选支持多选（美股 / 港股 / A股 / 加密 / 其他，市场为持仓上的标签） |
| FR-6.4 | 盈亏统一用平均成本法 |
...
| FR-9.12 | 界面语言 | v1 **仅中文**，但所有文案集中在一个 `i18n` 字典文件（不做语言切换，为 v2 预留） |
...
| 阶段 | 内容 | 交付判定 | 估算 |
|---|---|---|---|
| M0 能一键部署 | `setup-d1.cjs` / `setup-secrets.cjs` / `deploy:safe` / 两条部署路径 / README（含 FAQ）+ LICENSE + 截图（FR-9） | 陌生环境上一键部署成功；公开仓库可直接被 fork 使用 | 1.5 天 |
| Spike | 骨架 + setup/login + 账户 + 2 条持仓 + 饼图；实测 CPU time | 验证免费额度与认证方案可行 | 0.5–1 天 |
| M1 能录能用 | FR-1 / FR-2 / FR-3 / FR-4 | 能初始化、登录、建账户、录持仓、看审计 | 2.5–3.5 天 |
| M2 能看懂 | FR-5 / FR-6（饼图、treemap、明细、筛选联动） | 组合视图正确、筛选联动一致 | 1.5–2 天 |
| M3 能长期用 | FR-7 / FR-8 + 空状态 + 响应式 + 陈旧提示 | 导出导入闭环、切换币种正确 | 1.5–2 天 |
| 合计 | | | **约 8–10 人天** |
...
1. **每日走势图 + 每日快照 + Cron**（依赖 D3 移除项；`price_history`/`qty_history` 已在 v1 预埋）
2. **外部行情自动抓取**（CoinGecko/Binance/Yahoo/Finnhub 多源可插拔 + 失败降级 + 陈旧标记），并启用 FR-7.3 的开关
3. 自动汇率 + **汇率按日冻结**，消除 D9 的"改汇率平移历史/报表"
...
| 前端产物预算 | **≤ 100KB gzip（实测 81.5KB）** | 静态资源不计 CPU，不计请求额度 |
...
结论：**“简单” 体现为“少踩坑、少维护”，而不是“少跑一个 build”，所以选 React + Vite。** 同时用三条纪律守住“轻量”：手写 CSS、**手写图表（不引 ECharts/Chart.js）**、拒绝 UI 组件库 —— 实测整包 81.5KB gzip。
