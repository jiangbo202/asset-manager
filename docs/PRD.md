...
| FR-12.4 | 字典集中管理、零依赖 | 所有文案在 `src/shared/i18n.ts`，前后端共用同一份；插值用 `{{name}}` |
...
| 前端产物 | **首屏 101.5KB gzip + 按需分包**（v0.12 含中英文字典约 15KB；设置页 5.6KB / 持仓页 4.3KB 等按需加载） | 静态资源不计 CPU，不计请求额度 |
...
结论：**“简单” 体现为“少踩坑、少维护”，而不是“少跑一个 build”，所以选 React + Vite。** 同时用三条纪律守住“轻量”：手写 CSS、**手写图表（不引 ECharts/Chart.js）**、拒绝 UI 组件库。v0.12 加入中英文字典后首屏 101.5KB gzip（其中字典约 15KB）；字典是纯数据，如需再压可把非当前语言的字典改为动态 import。
