...
| **Workers CPU** | **10ms/请求，10ms/Cron** | ① 认证的 PBKDF2 放浏览器，Worker 只做一次 SHA-256（≈0.1ms）② 聚合在 SQL 里做，不在 JS 里遍历行 ③ 走势点服务端降采样到 ≤400 点 ④ 页面代码分包，减小解析量 ⑤ 定时任务分批刷新持仓（`CRON_MAX_HOLDINGS`，按最久未更新轮转），不让 CPU 随持仓数量无限增长 |
