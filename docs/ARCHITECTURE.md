...
| **Workers CPU** | **10ms/请求，10ms/Cron** | ① 认证的 PBKDF2 放浏览器，Worker 只做一次 SHA-256（≈0.1ms）② 聚合在 SQL 里做，不在 JS 里遍历行 ③ 走势点服务端降采样到 ≤400 点 ④ 页面代码分包，减小解析量 |
| Subrequests | 50/次调用 | 优先用批量接口（CoinGecko/Binance/腾讯）；Yahoo/自定义按标的单请求，用 6 并发 + 单次刷新请求预算上限 |
| 同时出站连接 | 6 | `mapLimit(items, 6, fn)` |
| Cron | 5 个/账号 | 只用 1 个（每小时）；非目标小时直接返回，不碰数据库以外的东西 |
...
providers.ts   适配器 + 代码映射（纯函数，易测）
  ...
  └─ 代码映射规则：700+hk→0700.HK / 600519+cn→600519.SS / BTC→bitcoin 或 BTCUSDT
index.ts       编排
...
3. `applyBackup`：拼装语句后写入
   ...
   - 跳过 `settings.schema_version`（那是库自身的结构标记）
