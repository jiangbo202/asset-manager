...
- **Cloudflare 用量**（设置页最底部，可选）：今天的 Workers 请求、D1 行读 / 行写、D1 库大小，对着免费额度显示 x/y 与进度条
...
**设置页最底部 → 「Cloudflare 用量」**，填一个**只读** API Token 就能看到今天的实际数字：

```
Workers 请求（今日）        512 / 100,000      ▏
D1 行读（今日）        4,120,000 / 5,000,000  ████████▏
D1 行写（今日）           96,500 / 100,000    ██████████▏  ← 超过 90% 变红
D1 数据库大小（asset-manager-db）  18.4 MB / 500 MB
```

Token 创建方式：控制台 → **My Profile → API Tokens → Create Token → Custom token**，权限**只勾两项只读**：

| 权限 | 用途 |
|---|---|
| Account → **Account Analytics** → Read | 读 Workers 请求数 |
| Account → **D1** → Read | 读 D1 行读/行写与库大小 |

> 为什么需要一个 Token：额度是**按账号**统计的，Worker 自己拿不到这些数字 ✗。
> Token 只读、加密存储、接口永不回传（连审计日志里也只留 `(set)` 标记），不填则整张卡只是"未配置"，其它功能完全不受影响。
> 数字缓存在本地，只有点「刷新用量」才去打 Cloudflare 的 API。
