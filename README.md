...
> 状态：**v0.12 — 功能已按 PRD 完成**（尚未在真实 Cloudflare 账号部署验证）。
...
- v0.13：CSV 导入（券商对账单）、交易日历感知（非交易日沿用收盘价）、更多币种的自动汇率
- v0.14：交易流水账本、FIFO / 分红 / 拆股、定投
- 更远：TOTP 二次验证、i18n、多用户 / 家庭共享

## 🌐 语言 / Language

界面支持 **简体中文 / English**，在「设置 → 界面语言」切换（默认跟随浏览器）。
服务端提示（表单校验、导入警告、行情失败原因、操作历史备注）也会按请求的 `Accept-Language` 返回对应语言。
字典集中在 `src/shared/i18n.ts`，新增文案只需在两份字典里各加一条 —— 测试会断言两种语言的 key 完全对齐，漏翻会直接失败。
