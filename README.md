...
> 状态：**v0.2（M0 + M1 已完成）**。
> 已实现：初始化/登录/改密码、账户（含 52 个内置平台图标选择器、归档）、持仓（含批量改价、归档）、
> 组合视图（环形图 + treemap + 明细表）、操作历史（字段级 diff + 日期筛选）、设置（显示币种 / 汇率 / 数据概览）。
> 未实现（见 [路线图](#路线图)）：备份导入导出、外部行情、每日走势。
...
- v0.2：JSON 备份导出 / 导入（含差异预览）、CSV 导入
