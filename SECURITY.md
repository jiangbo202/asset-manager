...
- 只有已登录的本人能读（`GET /api/settings/sessions` 在 `requireAuthOrPublicRead` 保护范围内，
  且**不在**公开只读白名单里 —— 匿名访客拿不到你的 IP 与设备）
- 返回给前端的 `id` 是 `sha256(会话 token)`，不是令牌本身：即使被第三方看到，也只能用来"吊销"，
  换不来登录态（token 是 32 字节随机值）
...
**分区分享**：可以只公开总览的一部分（开头 / 走势 / 分布 / 持仓明细），但裁剪在**服务端**做：

| 区域 | 公开后能看到 | 关闭后接口行为 |
|---|---|---|
| 开头 | 总资产、账户/持仓数、价格新鲜度、缺汇率提示 | `/api/portfolio` 里 `total`/`counts`/`staleDays` 置空 |
| 走势 | 每日净值曲线 | `/api/portfolio/history` → 401 |
| 分布 | 环形图与 treemap（类别/账户/币种/标的） | 三类分布清空、持仓只保留画图字段、`/api/accounts` → 401 |
| 持仓明细 | 数量、单价、平均成本、成本与盈亏 | 持仓字段长度抹平（数量 0、成本 null） |

关掉「分布」但开着「开头」时，`/api/portfolio` 连持仓列表都不返回 —— 前端"藏起来"只是界面礼貌，
接口才是边界。`tests/api/public-view.test.ts` 里直接搜原始响应文本，确认数量与成本没有出现在字节里。
